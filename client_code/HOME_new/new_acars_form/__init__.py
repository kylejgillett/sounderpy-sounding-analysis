...
class new_acars_form(new_acars_formTemplate):
  ...
  def acars_all_profiles_button_click(self, **event_args):
    ...
    self.acars_profiles_dropdown.placeholder = (
      "Profiles Loaded! Click here to browse and select a profile ID..."
    )
  ...
  def acars_button_click(self, **event_args):
    ...
    if len(self.acars_profiles_dropdown.selected_value) > 0:
      try:
        profile_idx = self.profiles_list.index(
          self.acars_profiles_dropdown.selected_value
        )
    ...
    args_list = [
      self.acars_profiles_dropdown.selected_value[0:8],
      year,
      month,
      day,
      hour,
